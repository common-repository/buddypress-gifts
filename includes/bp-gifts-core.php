...
if ( file_exists( dirname( __FILE__ ) . '/languages/buddypress-gifts-' . get_locale() . '.mo' ) )

	load_textdomain( 'bp-gifts', dirname( __FILE__ ) . '/languages/buddypress-gifts-' . get_locale() . '.mo' );
	...
	switch ( $action ) {
		...
		case 'new_gifts':
			...
			if ( (int)$total_items > 1 ) {
				...
				return apply_filters( 'bp_gifts_single_newgifts_notifications', '<a href="' . 'gifts' . '?new" title="' . $user_fullname .'\'s profile">' . sprintf( __( '%s sent you a gift!', 'bp-gifts' ), $user_fullname ) . '</a>', $user_fullname );
